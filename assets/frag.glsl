...
*/
#version 430
#define AMBIENT_OCCLUSION_ENABLED 1
...
void main() {
	...
	for (int i = 0; i < ANTIALIASING_SAMPLES; ++i) {
        ...
        float vignette;
			...
			if (col_r.w > 0.5) {
        #if MARBLE_DIFFUSE
          vec3 n = normalize(p.xyz - iMarblePos);
          float diff = max(dot(n, light_dir_n), 0.0);
          col += MARBLE_COLOR * LIGHT_COLOR * (diff + SHADOW_DARKNESS);
        #else
          ...
          #else
            col += refr * MARBLE_COLOR * 0.6 + refl * 0.4 + col_r.xyz;
          #endif
        #endif
			} else {
				col += col_r.xyz;
