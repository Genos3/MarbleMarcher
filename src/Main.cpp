...
static const float music_vol = 50.0f;
...
int main(int argc, char *argv[]) {
  ...
  sf::Context tempContext;
  if (!gladLoadGL()) {
    ERROR_MSG("GLAD failed to load");
    return 1;
  }
  ...
  init_globals(resolution);
  create_objects(render_tex.getNativeHandle());
  ...
  while (window.isOpen()) {
      ...
      if (event.type == sf::Event::Closed) {
        ...
        if (game_mode == CREDITS) {
          ...
          if (game_mode == MAIN_MENU) {
            ...
            scene.GetCurMusic().setVolume(GetVol());
            scene.SetExposure(1.0f);
            LockMouse(window, screen_center);
    ...
    if (lag_ms >= 1000.0f / target_fps) {
      ...
      sf::Sprite sprite(render_tex);
      sprite.setScale(
        float(screen_size.width) / float(resolution->width),
