...
static const float default_zoom = 20.0f;
...
void Scene::UpdateMarble(float dx, float dy) {
  ...
  if (free_camera) {
    ...
    #if SEA_ENABLED
      const bool is_underwater = marble_pos.y() < level_copy.water_y;
    #else
      const bool is_underwater = false;
    #endif
    
    //Add force from keyboard
    float f = marble_rad * (onGround ? ground_force : air_force);
    // if (is_underwater) { f *= 0.8f; }
    ...
    //Apply friction
    if (is_underwater) {
      // stronger friction underwater
      marble_vel *= 0.98f; // 0.93f
      
      // buoyancy - reduce gravity effect when underwater
      marble_vel.y() += marble_rad * gravity * 0.4f; // 0.6f
      
      // terminal velocity underwater
      const float max_speed = marble_rad * 0.2f; // 0.08f
      const float speed = marble_vel.norm();
      if (speed > max_speed) {
        marble_vel *= max_speed / speed;
      }
    } else {
